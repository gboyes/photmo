...
def main(target, corpus, outdir, maxcount, thresh):
    ...
    #x =cv2.cvtColor(cv2.imread(target), cv2.COLOR_BGR2HLS).astype(float) / 255.0
    ...
    for i in os.listdir(corpus):
        ...
        #y = cv2.cvtColor(cv2.imread(corpus + '/' + i), cv2.COLOR_BGR2HLS).astype(float) / 255.0
    ...
    while (m >= thresh) and (count <= maxcount):
        ...
        #cv2.imwrite("%s/iterdir/iteration%i.png"%(outdir, count), cv2.cvtColor((mod * 255).astype(np.uint8), cv2.COLOR_HLS2RGB))
...
if __name__ == "__main__":
    ...
    targs = '/Users/geb/Documents/Projects/FACE/putin-col'
    corp = ['/Users/geb/Documents/Projects/FACE/money/money']
    modp = '/Users/geb/Documents/Projects/FACE/money/money_out'
    ...
    for corpus in corp:
        ...
        for t in os.listdir(targs):
            ...
            main(targs + '/' + t, corpus, modname, 2000, 0.)
